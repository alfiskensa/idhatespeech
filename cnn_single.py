...
if __name__ == '__main__':
    ...
    if train:
        ...
        print('Test score: %.4f %%' % (score[1]*100))
        print(metrics.classification_report(test_labels, np.round(predictions[:, 0]).astype(float)))
        print('Precision score: %.4f %%' % (metrics.precision_score(test_labels, np.round(predictions[:, 0]).astype(float), average='weighted')*100))
        print('Recall score: %.4f %%' % (metrics.recall_score(test_labels, np.round(predictions[:, 0]).astype(float), average='weighted')*100))
        print('F1 score: %.4f %%' % (metrics.f1_score(test_labels, np.round(predictions[:, 0]).astype(float), average='weighted')*100))

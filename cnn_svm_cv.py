...
if __name__ == '__main__':
    ...
    #split dataset
    #X_train, X_test, y_train, y_test = train_test_split(tweets, labels, test_size=0.1)
    ...
    if train:
        for train, test in kfold.split(tweets, labels):
            ...
            print("Fold ",i)    
            model.fit(X_train, y_train, batch_size=128, epochs=16, validation_data=(X_val,y_val), callbacks=[checkpoint, reduce_lr])
            print("Testing...")
            score = model.evaluate(X_val, y_val, verbose=0)
            predicted = model.predict(X_val, batch_size=1024, verbose=1)
            predicted = np.round(predicted[:, 0]).astype(float)
            print('CNN Accuracy score: %.4f %%' % (score[1]*100))
            print(metrics.classification_report(y_val, predicted, digits=4))
            cnn_cvscores.append(score[1] * 100.0)
            cnn_precscores.append(metrics.precision_score(y_val, predicted, average='weighted')*100)
            cnn_recallscores.append(metrics.recall_score(y_val, predicted, average='weighted')*100)
            cnn_f1scores.append(metrics.f1_score(y_val, predicted, average='weighted')*100)
            #extract CNN feature
            svm_feats = model.predict(tweets, batch_size=1024, verbose=1)
            #spliting data
            ...
            cnnsvm_cvscores.append(accuracy * 100.0)
            cnnsvm_precscores.append(metrics.precision_score(svm_y_val, val_preds, average='weighted')*100)
            cnnsvm_recallscores.append(metrics.recall_score(svm_y_val, val_preds, average='weighted')*100)
            cnnsvm_f1scores.append(metrics.f1_score(svm_y_val, val_preds, average='weighted')*100)
            i = i+1
        print('Average Results:')
        print("CNN Accuracy Score: %.2f%% (+/- %.2f%%)" % (np.mean(cnn_cvscores), np.std(cnn_cvscores)))
        print('CNN Precision score: %.4f %%' % (np.mean(cnn_precscores)))
        print('CNN Recall score: %.4f %%' % (np.mean(cnn_recallscores)))
        print('CNN F1 score: %.4f %%' % (np.mean(cnn_f1scores)))
        print('\n')
        print("CNN-SVM Accuracy Score: %.2f%% (+/- %.2f%%)" % (np.mean(cnnsvm_cvscores), np.std(cnnsvm_cvscores)))
        print('CNN-SVM Precision score: %.4f %%' % (np.mean(cnnsvm_precscores)))
        print('CNN-SVM Recall score: %.4f %%' % (np.mean(cnnsvm_recallscores)))
        print('CNN-SVM F1 score: %.4f %%' % (np.mean(cnnsvm_f1scores)))
    else:
